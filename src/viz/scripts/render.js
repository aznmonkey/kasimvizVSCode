...
class ContactMap {
    ...
    setData(response) {
        ...
        let w = root.node().getBoundingClientRect().width * 20 - margin.left - margin.right;
        let h = w;//window.innerHeight - margin.top - margin.bottom;

        if (map.data) {
	    map.clearData();
            let layout = new Layout(map, new Dimension(w, h), margin);
            let renderer = new Render('temp', layout);
...
class Render {
    ...
    renderLinks() {
        ...
        let cluster =  d3.cluster()
            ...
            .size([360, innerRadius - nodeRadius/2]);
